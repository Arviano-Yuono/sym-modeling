...
def computeCauchyGreenStrain(F):
    ...
    C = np.concatenate((C11,C12,C21,C22),axis=1)
    return C
...
def computeStrainInvariantDerivatives(F,i,secondDerivative=False):
    ...
    if not(secondDerivative):
        dIdF = np.zeros((F.shape[0], F.shape[1]))
        if(i==1):
            # dI1/dF:
            dIdF = 2.0*F
        elif(i==2):
            ...
            dIdF = np.concatenate((dIdF11,dIdF12,dIdF21,dIdF22),axis=1)
        elif(i==3):
            # dI3/dF:
            J = F11*F22 - F12*F21
            dIdF11 = 2.0*F22 * J
    ...
    if secondDerivative:
        dIdF = np.zeros((F.shape[1], F.shape[1]))
        if(i==1):
            # d(dI1/dF)/dF:
            dIdF = 2.0*np.eye(F.shape[1])
        elif(i==3):
            # d(dI3/dF)/dF:
            J = F11*F22 - F12*F21
            dJdF11 = F22
