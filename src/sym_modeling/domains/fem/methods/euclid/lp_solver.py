...
# MODELS LIST:
#=====================================================================
from sym_modeling.domains.fem.methods.euclid.constraints import *
from sym_modeling.domains.fem.methods.euclid.solvers import *
...
def saveResultsLp(theta,c,counter_load=None):
    ...
    results_dir = getattr(c, "resultsDir", default_results_dir)
    ...
    f=open(resultfile,write_mode)
    f.write("-------------------- Data --------------------\n")
    if hasattr(c, "caseName"):
        f.write("Case Name: " + str(c.caseName) + "\n")
    if hasattr(c, "dataGroup"):
        f.write("Data Group: " + str(c.dataGroup) + "\n")
    if hasattr(c, "str_model"):
        f.write("Model: " + str(c.str_model) + "\n")
    if hasattr(c, "str_mesh"):
        f.write("Mesh: " + str(c.str_mesh) + "\n")
