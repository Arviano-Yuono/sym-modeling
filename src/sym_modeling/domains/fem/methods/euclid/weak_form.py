...
def computeInternalForceTheta(data,theta,c):
    ...
    for ele in range(data.numElements):                
        P_element = computeFirstPiolaTheta(data,theta,ele=ele)
...
def computeFirstPiolaTheta(data,theta,ele=None):
    """
    Compute the first Piola-Kirchhoff stress field for given material parameters.
    ...
    - `ele` - optional element number
    
    _Output Arguments_
    
    - `P` - first Piola-Kirchhoff stress at the quadrature points
    ...
    if ele is not None:
        ...
        return np.transpose(d_features_dF_element).dot(theta)

    P = np.zeros((data.numElements,4))
    for ele in range(data.numElements):
        P[ele,:] = computeFirstPiolaTheta(data,theta,ele=ele)
    return P
