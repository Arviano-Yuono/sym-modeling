...
class FEMReferenceState:
    def __init__(self):
        """
        Generate `Reference` object.
    
        """
        self.F = np.array([[1.0, 0.0, 0.0, 1.0]], dtype=float)
        ...
        self.P = np.array([[0.0, 0.0, 0.0, 0.0]], dtype=float)
...
#        self.dd_features_dI3dI3 = dd_features_dI3dI3 
    ...
    def convertTensorToNumpy(self,T):
        ...
        if isinstance(T, np.ndarray):
            return T
        if hasattr(T, "cpu") and hasattr(T, "detach"):
...
class FEMCaseData:
    def __init__(self, path,
        ...
        self.P = P #first Piola-Kirchhoff stress at quadrature points: numElements x 4
    ...
    def convertToNumpy(self):
        ...
        if self.P is not None:
            self.P = self.convertTensorToNumpy(self.P)


Reference = FEMReferenceState
Reaction = ReactionForce
FeatureSet = EuclidFeatureSet
FemDataset = FEMCaseData
